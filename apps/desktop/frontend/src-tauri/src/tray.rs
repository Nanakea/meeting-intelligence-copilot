...
pub fn create_tray<R: Runtime>(app: &AppHandle<R>) -> tauri::Result<()> {
    ...
    TrayIconBuilder::with_id("main-tray")
        .menu(&menu)
        .tooltip("Meeting Intelligence Copilot")
...
fn toggle_recording_handler<R: Runtime>(app: &AppHandle<R>) {
    ...
    tauri::async_runtime::spawn(async move {
        if crate::is_recording().await {
            ...
            // RecordingArgs is retained for command compatibility; recording
            // finalization uses the manager-owned folder, not this legacy path.
            ...
            // A late event-emission error can make the command return Err after
            // capture has stopped. Trust the authoritative recording state and
            // still let the frontend persist every transcript it received.
            if stop_result.is_ok() || !crate::is_recording().await {
                log::info!("Tray toggle: Recording stopped");
                if app_clone.emit("recording-stop-complete", true).is_err() {
                    log::error!("Tray toggle: Recording post-processing event was unavailable");
                }
            } else {
                log::error!("Tray toggle: Recording could not be stopped");
...
fn stop_recording_handler<R: Runtime>(app: &AppHandle<R>) {
    ...
    tauri::async_runtime::spawn(async move {
        ...
        if stop_result.is_ok() || !crate::is_recording().await {
            ...
            if app_clone.emit("recording-stop-complete", true).is_err() {
                log::error!("Tray: Recording post-processing event was unavailable");
            }
        } else {
            log::error!("Tray: Recording could not be stopped");
            update_tray_menu_async(&app_clone).await;
        }
    });
}
