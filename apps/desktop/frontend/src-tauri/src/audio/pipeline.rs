...
use super::recording_state::{
    observe_audio_pipeline_chunk_queued, observe_audio_pipeline_chunk_released, AudioChunk,
    AudioError, DeviceType, RecordingState,
};
...
struct AudioPipelineChunkGuard;

impl Drop for AudioPipelineChunkGuard {
    fn drop(&mut self) {
...
pub struct AudioCapture {
    ...
    #[allow(dead_code)] // Keeps the recording channel alive for the capture lifetime.
...
pub struct AudioPipeline {
    ...
    #[allow(dead_code)] // Preserves shared ownership for active pipeline state.
    state: Arc<RecordingState>,
    ...
    recording_sender_for_mixed: Option<super::recording_saver::RecordingAudioSender>,
}
...
impl AudioPipeline {
    ...
    pub async fn run(mut self) -> Result<()> {
        ...
        loop {
            ...
            {
                Ok(Some(chunk)) => {
                    let _queue_guard = AudioPipelineChunkGuard;
                    ...
                    while self.ring_buffer.can_mix() {
                        if let Some((mic_window, sys_window)) = self.ring_buffer.extract_window() {
                            ...
                            if let Some(ref sender) = self.recording_sender_for_mixed {
                                ...
                                if sender.send(recording_chunk).await.is_err() {
                                    debug!("Recording saver stopped accepting audio");
                                }
                            }
                        }
                    }
...
impl AudioPipelineManager {
    ...
    pub fn start(
        ...
        target_chunk_duration_ms: u32,
        sample_rate: u32,
        recording_sender: Option<super::recording_saver::RecordingAudioSender>,
        mic_device_name: String,
        mic_device_kind: super::device_detection::InputDeviceKind,
        system_device_name: String,
        system_device_kind: super::device_detection::InputDeviceKind,
        ...
        if let Some(sender) = &self.audio_sender {
            ...
            if let Err(e) = sender.send(flush_chunk) {
                ...
                for i in 0..3 {
                    ...
                    observe_audio_pipeline_chunk_queued();
                    if sender.send(additional_flush).is_err() {
                        observe_audio_pipeline_chunk_released();
                    }
                }
