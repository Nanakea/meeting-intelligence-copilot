...
/// Creates structure: base_path/MeetingName_YYYY-MM-DD_HH-MM_<opaque suffix>/
...
pub fn create_meeting_folder(
    ...
    if let Err(error) = crate::meeting_local_data::mark_meeting_folder(&meeting_folder) {
    ...
    if create_checkpoints_dir {
        ...
        if let Err(error) = std::fs::create_dir_all(&checkpoints_dir) {
            let _ = std::fs::remove_dir_all(&meeting_folder);
            return Err(error.into());
        }
        log::info!("Created owned local meeting folder with checkpoints");
    } else {
        log::info!("Created owned local meeting folder without checkpoints");
    }
