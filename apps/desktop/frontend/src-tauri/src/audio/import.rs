...
pub async fn start_import<R: Runtime>(
    ...
    // Import, recording start/stop, and destructive local-data operations share
    // one lifecycle lock so a newly created folder can never be swept mid-job.
    ...
    super::common::unload_engine_after_batch_while_locked(use_parakeet).await;
    ...
    match &result {
        ...
        Err(_) => {
            let _ = app.emit(
                "import-error",
                ImportError {
                    error: "The local audio import could not be completed".to_string(),
                },
            );
        }
    }
...
async fn run_import<R: Runtime>(
    ...
    info!(
        "Starting local audio import (language_configured: {}, model_configured: {}, provider_configured: {})",
        language.is_some(),
        model.is_some(),
        provider.is_some()
    );
    ...
    info!("Copied imported audio into owned local meeting storage");
    ...
    if write_transcripts_json(&meeting_folder, &segments).is_err() {
        warn!("Failed to write imported transcript data");
    }

    if write_import_metadata(
        &meeting_folder,
        &meeting_id,
        &title,
        duration_seconds,
    ...
    .is_err()
    {
        warn!("Failed to write imported meeting metadata");
    }
    ...
    match engine {
        Some(e) => {
            ...
            if needs_load {
                ...
                if e.discover_models().await.is_err() {
                    warn!("Local model discovery failed; import will continue with configured defaults");
                }
...
fn write_import_metadata(
    ...
    info!("Wrote imported-meeting metadata");
    Ok(())
}
...
pub async fn select_and_validate_audio_command<R: Runtime>(
    ...
    match file_path {
        Some(path) => {
            ...
            info!("User selected a local audio file for validation");

            match validate_audio_file(Path::new(&path_str)) {
                Ok(info) => Ok(Some(info)),
                Err(_) => {
                    error!("Selected local audio file validation failed");
                    Err("The selected audio file could not be validated".to_string())
                }
            }
        }
        None => {
...
pub async fn validate_audio_file_command(path: String) -> Result<AudioFileInfo, String> {
    info!("Validating a selected local audio file");
    validate_audio_file(Path::new(&path))
        .map_err(|_| "The selected audio file could not be validated".to_string())
}
...
pub async fn start_import_audio_command<R: Runtime>(
    ...
    tauri::async_runtime::spawn(async move {
        ...
        if result.is_err() {
            error!("Local audio import failed");
        }
    });
