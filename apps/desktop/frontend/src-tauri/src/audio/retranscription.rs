        ...
        Err(_) => {
            let _ = app.emit(
                ...
                RetranscriptionError {
                    ...
                    error: "The local meeting could not be retranscribed".to_string(),
                },
            );
        }
    }
...
async fn run_retranscription<R: Runtime>(
    ...
    if write_transcripts_json(&folder_path, &segments).is_err() {
        warn!("Failed to write retranscribed meeting data");
    }
    ...
    if write_retranscription_metadata(&folder_path, &meeting_id, duration_seconds, &audio_filename)
        .is_err()
    {
        warn!("Failed to update retranscription metadata");
    }
...
async fn get_configured_whisper_model<R: Runtime>(app: &AppHandle<R>) -> Result<String> {
    ...
    let result: Option<(String, String)> =
        sqlx::query_as("SELECT provider, model FROM transcript_settings WHERE id = '1'")
            ...
            .await
            .map_err(|_| {
                error!("Failed to query local transcript configuration");
                anyhow!("Failed to query transcript config")
            })?;
...
fn write_retranscription_metadata(
    ...
    info!("Wrote retranscription metadata");
    Ok(())
}
...
pub async fn start_retranscription_command<R: Runtime>(
    ...
    tauri::async_runtime::spawn(async move {
        ...
        if result.is_err() {
            error!("Local retranscription failed");
        }
    });
