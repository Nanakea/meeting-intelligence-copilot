...
static TRANSCRIPTION_QUEUE_DEPTH: AtomicUsize = AtomicUsize::new(0);
static MAX_TRANSCRIPTION_QUEUE_DEPTH: AtomicUsize = AtomicUsize::new(0);
...
pub(crate) fn observe_transcription_chunk_queued() {
    let depth = TRANSCRIPTION_QUEUE_DEPTH.fetch_add(1, Ordering::Relaxed) + 1;
    MAX_TRANSCRIPTION_QUEUE_DEPTH.fetch_max(depth, Ordering::Relaxed);
    TRANSCRIPTION_LAST_ACTIVITY_MS.store(activity_timestamp_ms(), Ordering::Relaxed);
}

pub(crate) fn observe_transcription_chunk_released() {
    let _ = TRANSCRIPTION_QUEUE_DEPTH.fetch_update(Ordering::Relaxed, Ordering::Relaxed, |depth| {
        Some(depth.saturating_sub(1))
    });
    TRANSCRIPTION_LAST_ACTIVITY_MS.store(activity_timestamp_ms(), Ordering::Relaxed);
}

struct TranscriptionChunkGuard;

impl Drop for TranscriptionChunkGuard {
    fn drop(&mut self) {
        observe_transcription_chunk_released();
    }
}

pub(crate) fn reset_transcription_queue_diagnostics() {
    TRANSCRIPTION_QUEUE_DEPTH.store(0, Ordering::Relaxed);
    MAX_TRANSCRIPTION_QUEUE_DEPTH.store(0, Ordering::Relaxed);
    TRANSCRIPTION_LAST_ACTIVITY_MS.store(0, Ordering::Relaxed);
}

pub fn transcription_queue_snapshot() -> TranscriptionQueueSnapshot {
    let last_activity_at = TRANSCRIPTION_LAST_ACTIVITY_MS.load(Ordering::Relaxed);
    TranscriptionQueueSnapshot {
        current_depth: TRANSCRIPTION_QUEUE_DEPTH.load(Ordering::Relaxed),
        max_depth: MAX_TRANSCRIPTION_QUEUE_DEPTH.load(Ordering::Relaxed),
        last_activity_ms: if last_activity_at == 0 {
            0
        } else {
            activity_timestamp_ms().saturating_sub(last_activity_at)
        },
    }
}
...
/// Sequence IDs are recording-local and must restart with each opaque session.
pub fn reset_sequence_counter() {
    SEQUENCE_COUNTER.store(0, Ordering::SeqCst);
}

/// Allocate the next recording-local transcript sequence ID.
...
pub fn start_transcription_task<R: Runtime>(
    ...
    tokio::spawn(async move {
        ...
        let transcription_engine = match super::engine::get_or_init_transcription_engine(&app).await
        {
            Ok(engine) => engine,
            Err(_) => {
                error!("Failed to initialize the configured transcription engine");
        ...
        for worker_id in 0..NUM_WORKERS {
            ...
            let worker_handle = tokio::spawn(async move {
                ...
                loop {
                    ...
                    match chunk {
                        Some(chunk) => {
                            let _queue_guard = TranscriptionChunkGuard;
                                ...
                                .await
                            {
                                Ok((transcript, confidence_opt, is_partial)) => {
                                    ...
                                    if !transcript.trim().is_empty() && meets_threshold {
                                        ...
                                        info!(
                                            "✅ Worker {} transcribed segment (chars: {}, confidence: {}, partial: {})",
                                            worker_id,
                                            transcript.chars().count(),
                                            confidence_str,
                                            is_partial
                                        ...
                                        if !current_flag {
                                            ...
                                            match app_clone.emit("speech-detected", serde_json::json!({
                                                ...
                                                Err(_) => error!("🎤 ❌ Failed to emit speech-detected event"),
                                            }
                                        } else {
                                        ...
                                        // [DISPOSABLE SPIKE TAP] append the REAL emitted TranscriptUpdate as
                                        // one JSON object per line to a dedicated JSONL capture file, at the
                                        // emission boundary, WITHOUT altering the Tauri event emission below.
                                        // Scratch clone only, acceptance-test infrastructure -- NOT part of
                                        // normal product use (independent review finding, confirmed real: this
                                        // used to always run, falling back to a %TEMP% path, adding blocking
                                        // sync file I/O to the hot transcription path unconditionally and
                                        // persisting transcript data even when nobody asked for it). Now strictly
                                        // opt-in: only runs when SPIKE_TAP_PATH is explicitly set, no fallback.
                                        if let Ok(__tap_path) = std::env::var("SPIKE_TAP_PATH") {
                                            if let Ok(__spike_json) = serde_json::to_string(&update)
                                            {
                                                ...
                                                {
                                                    use std::io::Write;
                                                    let _ = writeln!(__f, "{}", __spike_json);
                                                    let _ = __f.flush();
                                                }
                                            }
                                        }

                                        let persisted = crate::audio::recording_commands::
                                            persist_transcript_update_internal(&update);
                                        if !persisted {
                                            warn!(
                                                "Worker {} could not commit transcript history",
                                                worker_id
                                            );
                                        }
                                        if app_clone.emit("transcript-update", &update).is_err() {
                                            error!(
                                                "Worker {}: Failed to emit transcript update",
                                                worker_id
                                            );
                                        }
                                        // Publish to canonical recording history before transport can
                                        // acknowledge it. Full backend reset repair replays this history.
                                        if persisted {
                                            if let Some(dispatcher) = app_clone.try_state::<
                                                super::intelligence_dispatcher::IntelligenceDispatcherState,
                                            >() {
                                                if !dispatcher.enqueue(update.clone()) {
                                                    log::debug!(
                                                        "Meeting Intelligence dispatcher unavailable; recording continues"
                                                    );
                                                }
                                            }
                                        }
                                ...
                                Err(e) => {
                                    ...
                                    match e {
                                        TranscriptionError::AudioTooShort { .. } => {
                                            ...
                                            info!(
                                                "Worker {} skipped a short audio chunk",
                                                worker_id
                                            );
                                            chunks_completed_clone.fetch_add(1, Ordering::SeqCst);
                                            continue;
                                        ...
                                        _ => {
                                            warn!("Worker {}: Transcription failed", worker_id);
                                            let _ = app_clone.emit(
                                                "transcription-warning",
                                                serde_json::json!({
                                                    "error": "transcription_failed",
                                                    "userMessage": "Local speech recognition failed for an audio segment.",
                                                    "actionable": false
                                                }),
                                            );
                                        }
        ...
        for (worker_id, handle) in worker_handles.into_iter().enumerate() {
            if handle.await.is_err() {
                error!("❌ Worker {} stopped unexpectedly", worker_id);
            } else {
    ...
    match engine {
        TranscriptionEngine::Whisper(whisper_engine) => {
                ...
                .await
            {
                ...
                Err(_) => {
                    error!("Whisper transcription failed for chunk {}", chunk.chunk_id);

                    let transcription_error =
                        TranscriptionError::EngineFailed("local_speech_engine_failed".to_string());
                    let _ = app.emit(
                        "transcription-error",
                        &serde_json::json!({
                            "error": "transcription_failed",
        ...
        TranscriptionEngine::Parakeet(parakeet_engine) => {
            match parakeet_engine.transcribe_audio(speech_samples).await {
                ...
                Err(_) => {
                    error!("Parakeet transcription failed for chunk {}", chunk.chunk_id);
            ...
            match provider.transcribe(speech_samples, language).await {
                ...
                Err(_) => {
                    ...
                    Err(TranscriptionError::EngineFailed(
                        "local_speech_engine_failed".to_string(),
                    ))
                }
            }
        }
...
mod sequence_tests {
    use super::{
        next_sequence_id, observe_transcription_chunk_queued, observe_transcription_chunk_released,
        reset_sequence_counter, reset_transcription_queue_diagnostics,
        transcription_queue_snapshot,
    };

    #[test]
    fn recording_sequence_resets_before_the_next_session() {
        reset_sequence_counter();
        assert_eq!(next_sequence_id(), 0);
    ...
    #[test]
    fn transcription_queue_diagnostics_track_depth_and_reset() {
        reset_transcription_queue_diagnostics();
        assert_eq!(transcription_queue_snapshot().current_depth, 0);

        observe_transcription_chunk_queued();
        observe_transcription_chunk_queued();
        observe_transcription_chunk_released();

        let snapshot = transcription_queue_snapshot();
        ...
        reset_transcription_queue_diagnostics();
        let reset = transcription_queue_snapshot();
        assert_eq!(reset.current_depth, 0);
        assert_eq!(reset.max_depth, 0);
        assert_eq!(reset.last_activity_ms, 0);
    }
}
