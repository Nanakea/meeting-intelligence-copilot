...
const SILENCE_TRIM_INTERVAL_SAMPLES: usize = 2 * 16_000;
...
pub struct ContinuousVadProcessor {
    ...
    last_silence_trim_sample: usize,
...
impl ContinuousVadProcessor {
    ...
    fn process_chunk(&mut self, chunk: &[f32]) -> Result<()> {
        ...
        // silero_rs intentionally retains session audio until its caller trims
        // silence or receives SpeechEnd. Without this, a quiet recording grows
        // the native VAD buffer for the entire meeting.
        if !self.in_speech
            && self.session.current_silence_duration() >= Duration::from_secs(2)
            && self
                .processed_samples
                .saturating_sub(self.last_silence_trim_sample)
                >= SILENCE_TRIM_INTERVAL_SAMPLES
        {
            self.session.trim_start_silence();
            self.last_silence_trim_sample = self.processed_samples;
        }
...
mod tests {
    ...
    #[test]
    fn continuous_silence_keeps_only_bounded_vad_context() {
        let mut processor =
            ContinuousVadProcessor::new(16000, 2000).expect("Failed to create processor");
        let silence = vec![0.0f32; 12 * 16000];

        for chunk in silence.chunks(1600) {
            assert!(processor.process_audio(chunk).unwrap().is_empty());
        }

        let (buffer_start, buffer_end) = processor.session.current_buffer_range();
        assert!(
            buffer_end.saturating_sub(buffer_start) <= Duration::from_millis(2300),
            "silent VAD context was not bounded: {:?}",
            buffer_end.saturating_sub(buffer_start)
        );
    }
