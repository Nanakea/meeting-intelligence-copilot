...
pub fn calculate_buffer_timeout(
    ...
    // Calculate the 2x jitter budget in integer microseconds. Ceiling division
    // avoids both floating-point drift and an unsafe early timeout when the
    // sample count does not divide evenly by the sample rate.
    let numerator = u64::from(buffer_size) * 2 * 1_000_000;
    let denominator = u64::from(sample_rate);
    let timeout_micros = numerator.div_ceil(denominator);
    let with_headroom = Duration::from_micros(timeout_micros);
...
mod tests {
    ...
    #[test]
    fn test_calculate_buffer_timeout_rounds_fractional_microseconds_up() {
        let timeout = calculate_buffer_timeout(InputDeviceKind::Wired, 1024, 48000);

        assert_eq!(timeout, Duration::from_micros(42_667));
    }
