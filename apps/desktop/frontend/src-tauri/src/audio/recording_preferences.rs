...
pub async fn load_recording_preferences<R: Runtime>(
    ...
    let store = match app.store("recording_preferences.json") {
        Ok(store) => store,
        Err(_) => {
            warn!("Failed to access recording preference store; using defaults");
    ...
    let prefs = if let Some(value) = store.get("preferences") {
        match serde_json::from_value::<RecordingPreferences>(value.clone()) {
            ...
            Err(_) => {
                warn!("Failed to deserialize recording preferences; using defaults");
    ...
    info!(
        "Loaded recording preferences: custom_save_folder={}, auto_save={}, format={}, mic_configured={}, system_configured={}",
...
pub async fn save_recording_preferences<R: Runtime>(
    ...
    info!(
        "Saving recording preferences: custom_save_folder={}, auto_save={}, format={}, mic_configured={}, system_configured={}",
    ...
    let store = app
        ...
        .map_err(|_| anyhow::anyhow!("Failed to access recording preference store"))?;

    // Serialize preferences to JSON value
    let prefs_value = serde_json::to_value(preferences)
        .map_err(|_| anyhow::anyhow!("Failed to serialize recording preferences"))?;
    ...
    store
        .save()
        .map_err(|_| anyhow::anyhow!("Failed to save recording preferences"))?;
...
pub async fn get_recording_preferences<R: Runtime>(
    ...
    load_recording_preferences(&app)
        .await
        .map_err(|_| "Failed to load recording preferences".to_string())
}
...
pub async fn set_recording_preferences<R: Runtime>(
    ...
    save_recording_preferences(&app, &preferences)
        .await
        .map_err(|_| "Failed to save recording preferences".to_string())
}
...
pub async fn open_recordings_folder<R: Runtime>(app: AppHandle<R>) -> Result<(), String> {
    ...
    ensure_recordings_directory(&preferences.save_folder)
        .map_err(|_| "Failed to prepare the recordings directory".to_string())?;
    ...
    #[cfg(target_os = "windows")]
    {
        std::process::Command::new("explorer")
            .arg(&folder_path)
            .spawn()
            .map_err(|_| "Failed to open the recordings folder".to_string())?;
    }
    ...
    info!("Opened the configured local recordings folder");
    Ok(())
}
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        if backend_enum == AudioCaptureBackend::CoreAudio {
            ...
            if !check_screen_recording_permission() {
                ...
                if request_screen_recording_permission().is_err() {
                    error!("Failed to open system audio-capture settings");
                }
