...
static AUDIO_PIPELINE_QUEUE_DEPTH: AtomicUsize = AtomicUsize::new(0);
static MAX_AUDIO_PIPELINE_QUEUE_DEPTH: AtomicUsize = AtomicUsize::new(0);

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct AudioPipelineQueueSnapshot {
    pub current_depth: usize,
    pub max_depth: usize,
}

pub(crate) fn reset_audio_pipeline_queue_diagnostics() {
    AUDIO_PIPELINE_QUEUE_DEPTH.store(0, Ordering::Relaxed);
    MAX_AUDIO_PIPELINE_QUEUE_DEPTH.store(0, Ordering::Relaxed);
}

pub(crate) fn observe_audio_pipeline_chunk_queued() {
    let depth = AUDIO_PIPELINE_QUEUE_DEPTH.fetch_add(1, Ordering::Relaxed) + 1;
    MAX_AUDIO_PIPELINE_QUEUE_DEPTH.fetch_max(depth, Ordering::Relaxed);
}

pub(crate) fn observe_audio_pipeline_chunk_released() {
    let _ =
        AUDIO_PIPELINE_QUEUE_DEPTH.fetch_update(Ordering::Relaxed, Ordering::Relaxed, |depth| {
            Some(depth.saturating_sub(1))
        });
}

pub fn audio_pipeline_queue_snapshot() -> AudioPipelineQueueSnapshot {
    AudioPipelineQueueSnapshot {
        current_depth: AUDIO_PIPELINE_QUEUE_DEPTH.load(Ordering::Relaxed),
        max_depth: MAX_AUDIO_PIPELINE_QUEUE_DEPTH.load(Ordering::Relaxed),
    }
}
...
impl RecordingState {
    ...
    pub fn send_audio_chunk(&self, chunk: AudioChunk) -> Result<()> {
        ...
        if let Some(sender) = self.audio_sender.lock().unwrap().as_ref() {
            observe_audio_pipeline_chunk_queued();
            if sender.send(chunk).is_err() {
                observe_audio_pipeline_chunk_released();
                return Err(anyhow::anyhow!("Failed to send audio chunk"));
            }
...
#[cfg(test)]
mod queue_diagnostics_tests {
    use super::{
        audio_pipeline_queue_snapshot, observe_audio_pipeline_chunk_queued,
        observe_audio_pipeline_chunk_released, reset_audio_pipeline_queue_diagnostics,
    };

    #[test]
    fn audio_pipeline_queue_diagnostics_track_depth_and_reset() {
        reset_audio_pipeline_queue_diagnostics();
        observe_audio_pipeline_chunk_queued();
        observe_audio_pipeline_chunk_queued();
        observe_audio_pipeline_chunk_released();

        let snapshot = audio_pipeline_queue_snapshot();
        assert_eq!(snapshot.current_depth, 1);
        assert_eq!(snapshot.max_depth, 2);

        observe_audio_pipeline_chunk_released();
        observe_audio_pipeline_chunk_released();
        assert_eq!(audio_pipeline_queue_snapshot().current_depth, 0);

        reset_audio_pipeline_queue_diagnostics();
        let reset = audio_pipeline_queue_snapshot();
