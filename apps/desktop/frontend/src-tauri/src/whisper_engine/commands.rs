...
use crate::whisper_engine::{ModelInfo, WhisperEngine, WhisperModelReadiness};
...
pub fn set_models_directory<R: Runtime>(app: &AppHandle<R>) {
    ...
    log::info!("Whisper models directory configured");
...
pub async fn whisper_get_available_models() -> Result<Vec<ModelInfo>, String> {
    ...
    if let Some(engine) = engine {
        ...
        tokio::task::spawn_blocking(discover_models_standalone)
            .await
            .map_err(|_| "Whisper model scan could not complete".to_string())?
    }
}
...
fn discover_models_standalone() -> Result<Vec<ModelInfo>, String> {
    use crate::whisper_engine::{
        approved_artifact, inspect_artifact_file, ModelStatus, WhisperModelIntegrity,
    };
    ...
    log::info!("Scanning configured Whisper models directory");
    ...
    for &(name, filename, size_mb, accuracy, speed, description) in model_configs {
        ...
        let status = match approved_artifact(name) {
            Ok((_, artifact)) => match inspect_artifact_file(&model_path, &artifact) {
                ...
                _ => ModelStatus::Corrupted {
                    file_size: model_path
                        ...
                        .map(|value| value.len())
                        .unwrap_or_default(),
                    expected_min_size: artifact.byte_count,
                },
            },
...
pub async fn whisper_verified_model_readiness(
    model_name: &str,
    language: Option<&str>,
    ...
    match engine {
        Some(engine) => engine.verified_model_readiness(model_name, language).await,
        None => {
            crate::whisper_engine::model_readiness(
                model_name,
                PathBuf::from(".whisper-engine-unavailable"),
                language,
            )
            .await
        }
...
pub async fn whisper_download_model(
    ...
    if let Some(engine) = engine {
        ...
        let progress_callback: Box<dyn Fn(u8) + Send + Sync> = Box::new(move |progress: u8| {
        ...
        match result {
            ...
            Err(_) => {
                ...
                if let Err(emit_e) = app_handle.emit(
                    ...
                    serde_json::json!({
                        "modelName": model_name,
                        "error": "verified_download_failed"
                    }),
                ) {
                    log::error!("Failed to emit download error event: {}", emit_e);
                }
                Err("The verified speech model could not be installed".to_string())
            }
        }
    } else {
        Err("Whisper engine not initialized".to_string())
