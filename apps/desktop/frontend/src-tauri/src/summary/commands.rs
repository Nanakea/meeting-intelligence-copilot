...
pub async fn api_save_meeting_summary<R: Runtime>(
    ...
    match SummaryProcessesRepository::update_meeting_summary(pool, &meeting_id, &summary).await {
        ...
        Err(_) => {
            log_error!("Failed to save meeting summary for {}", meeting_id);
            Err("Failed to save meeting summary".to_string())
        }
    }
}
...
async fn resolve_meeting_folder(
    ...
    let meeting = MeetingsRepository::get_meeting_metadata(pool, meeting_id)
        .await
        .map_err(|_| "Failed to load saved meeting metadata".to_string())?
        .ok_or_else(|| "Saved meeting not found".to_string())?;
...
pub async fn api_get_summary<R: Runtime>(
    ...
    match SummaryProcessesRepository::get_summary_data_for_meeting(pool, &meeting_id).await {
        Ok(Some(process)) => {
            ...
            let data = if let Some(result_str) = process.result {
                match serde_json::from_str::<serde_json::Value>(&result_str) {
                    ...
                    Err(_) => {
                        log_error!("Failed to parse saved summary result JSON");
                        None
                    }
                }
            } else {
            ...
            let meeting_name = match MeetingsRepository::get_meeting(pool, &meeting_id).await {
                Ok(Some(meeting_details)) => {
                    log_info!("Fetched saved meeting title");
                ...
                Err(_) => {
                    log_error!("Failed to fetch saved meeting title");
                    None
                }
            };
        ...
        Err(_) => {
            log_error!("Error retrieving summary for {}", meeting_id);
            Err("Failed to retrieve saved summary".to_string())
        }
    }
}
...
pub async fn api_process_transcript<R: Runtime>(
    ...
    let references_saved_meeting = meeting_id.is_some();
    ...
    if references_saved_meeting {
        let exists = MeetingsRepository::get_meeting_metadata(&pool, &m_id)
            .await
            .map_err(|_| {
                "The saved meeting could not be prepared for summary generation".to_string()
            })?
            .is_some();
        if !exists {
            return Err("The saved meeting no longer exists".to_string());
        }
    }
    ...
    SummaryProcessesRepository::create_or_reset_process(&pool, &m_id)
        .await
        .map_err(|_| "Summary generation could not be initialized".to_string())?;
    ...
    .await
    .map_err(|_| "Transcript data could not be prepared for summary generation".to_string())?;

    log_info!("✓ Transcript chunks saved for meeting_id: {}", &m_id);

    // Register synchronously while the local-data lifecycle lock is held. A
    // concurrent delete can now cancel this exact generation before removing
    // its rows, and an older generation cannot unregister its replacement.
    let (cancellation_token, cancellation_registration) =
        SummaryService::register_cancellation_token(&m_id);
...
pub async fn api_cancel_summary<R: Runtime>(
    ...
    if cancelled {
        ...
        if SummaryProcessesRepository::update_process_cancelled(pool, &meeting_id)
            ...
            .is_err()
        {
            log_error!("Failed to update DB status to cancelled for {}", meeting_id);
            return Err("Failed to update summary cancellation status".to_string());
        }
