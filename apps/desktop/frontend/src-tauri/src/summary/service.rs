...
pub(crate) struct SummaryCancellationRegistration {
    meeting_id: String,
    registration_id: Uuid,
}

impl Drop for SummaryCancellationRegistration {
    fn drop(&mut self) {
        if let Ok(mut registry) = CANCELLATION_REGISTRY.lock() {
            let owns_registration = registry
                .get(&self.meeting_id)
                .is_some_and(|active| active.registration_id == self.registration_id);
            if owns_registration {
                registry.remove(&self.meeting_id);
                info!(
                    "Cleaned up cancellation token for meeting: {}",
                    self.meeting_id
                );
            }
        }
...
// Global registry for active summary generations (thread-safe).
static CANCELLATION_REGISTRY: Lazy<Arc<Mutex<HashMap<String, ActiveSummary>>>> =
...
impl SummaryService {
    ...
    pub(crate) fn register_cancellation_token(
        meeting_id: &str,
    ) -> (CancellationToken, SummaryCancellationRegistration) {
        ...
        (
            token,
            SummaryCancellationRegistration {
    ...
    pub fn cancel_summary(meeting_id: &str) -> bool {
        if let Ok(registry) = CANCELLATION_REGISTRY.lock() {
            if let Some(active) = registry.get(meeting_id) {
    ...
    pub fn cancel_all_summaries() -> usize {
    ...
    pub(crate) async fn process_transcript_background<R: tauri::Runtime>(
        ...
        _cancellation_registration: SummaryCancellationRegistration,
        text: String,
        model_provider: String,
        model_name: String,
            ...
            || provider == LLMProvider::CustomOpenAI
        {
            ...
            match SettingsRepository::get_api_key(&pool, &model_provider).await {
                ...
                Err(_) => {
                    let err_msg = format!("Failed to retrieve API key for {}", &model_provider);
                    Self::update_process_failed(&pool, &meeting_id, &err_msg).await;
                    return;
                }
            }
        ...
        let ollama_endpoint = if provider == LLMProvider::Ollama {
            match SettingsRepository::get_model_config(&pool).await {
                ...
                Ok(None) => None,
                Err(_) => {
                    info!("Failed to retrieve Ollama endpoint; using default");
                    None
                }
            }
        } else {
        ...
        let (
            ...
            match SettingsRepository::get_custom_openai_config(&pool).await {
                Ok(Some(config)) => {
                    info!("Using configured custom OpenAI endpoint");
                    (
                ...
                Err(_) => {
                    let err_msg = "Failed to retrieve custom OpenAI config".to_string();
                    Self::update_process_failed(&pool, &meeting_id, &err_msg).await;
                    return;
                }
            }
        ...
        let token_threshold = if provider == LLMProvider::Ollama {
            ...
            match model {
                ...
                Err(_) => {
                    warn!("Unknown built-in summary model metadata; using default 2048");
        ...
        let template = match templates::get_template(&template_id) {
            ...
            Err(_) => {
                let err_msg = format!("Failed to load summary template '{}'", template_id);
                Self::update_process_failed(&pool, &meeting_id, &err_msg).await;
                return;
            }
        };
        ...
        match result {
            Ok((final_markdown, english_markdown, num_chunks)) => {
                // Serialize completion with retention/delete commands. If deletion
                // won the race, its cancellation is visible before any meeting or
                // summary row can be written again.
                let _local_data_guard = crate::audio::common::acquire_engine_lifecycle_lock().await;
                if cancellation_token.is_cancelled() {
                    info!(
                        "Summary completion discarded after cancellation for meeting_id: {}",
                        meeting_id
                    );
                    return;
                }
                ...
                {
                    info!("Extracted a meeting name from the completed summary");
                    if MeetingsRepository::update_meeting_name(&pool, &meeting_id, &name)
                        .await
                        .is_err()
                    {
                        error!("Failed to update meeting name for {}", meeting_id);
                    } else {
                ...
                if SummaryProcessesRepository::update_process_completed(
                    &pool,
                    &meeting_id,
                    result_json,
                    num_chunks,
                ...
                .is_err()
                {
                    error!("Failed to save completed process for {}", meeting_id);
                } else {
            ...
            Err(e) => {
                ...
                if e.contains("cancelled") {
                    ...
                    if SummaryProcessesRepository::update_process_cancelled(&pool, &meeting_id)
                        .await
                        .is_err()
                    {
                        error!("Failed to update DB status to cancelled for {}", meeting_id);
                    }
                } else {
                    Self::update_process_failed(
                        &pool,
                        &meeting_id,
                        "Summary generation failed; check the selected model and provider settings",
                    )
                    .await;
                }
            }
    ...
    async fn update_process_failed(pool: &SqlitePool, meeting_id: &str, error_msg: &str) {
        error!("Processing failed for meeting_id {}", meeting_id);
        if SummaryProcessesRepository::update_process_failed(pool, meeting_id, error_msg)
            .await
            .is_err()
        {
            error!("Failed to update DB status to failed for {}", meeting_id);
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn replacement_summary_registration_cannot_be_removed_by_older_generation() {
        let meeting_id = format!("meeting-summary-registry-{}", Uuid::new_v4().simple());
        let (first_token, first_registration) =
            SummaryService::register_cancellation_token(&meeting_id);
        let (second_token, second_registration) =
            SummaryService::register_cancellation_token(&meeting_id);

        assert!(first_token.is_cancelled());
        drop(first_registration);
        assert!(SummaryService::cancel_summary(&meeting_id));
