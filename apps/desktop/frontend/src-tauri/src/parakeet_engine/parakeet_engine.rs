...
impl std::fmt::Display for ParakeetEngineError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ParakeetEngineError::TranscriptionFailed(_) => write!(f, "Transcription failed"),
            ParakeetEngineError::DownloadFailed(_) => write!(f, "Model download failed"),
            ParakeetEngineError::IoError(_) => write!(f, "Local model file operation failed"),
            ParakeetEngineError::Other(_) => write!(f, "Parakeet operation failed"),
        }
    }
}
...
impl ParakeetEngine {
    ...
    pub async fn download_model_detailed(
        ...
        // Network installation is disabled at the command boundary. Keep any
        // inherited internal call loopback-only as defense in depth.
        let base_url = "http://127.0.0.1:9/parakeet-download-disabled";
