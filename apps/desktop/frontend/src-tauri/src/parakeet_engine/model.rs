...
const ENABLE_CPU_MEMORY_ARENA: bool = false;
const ENABLE_MEMORY_PATTERN: bool = false;

#[cfg(test)]
mod memory_policy_tests {
    use super::{ENABLE_CPU_MEMORY_ARENA, ENABLE_MEMORY_PATTERN};

    #[test]
    fn live_variable_length_inference_does_not_retain_ort_shape_caches() {
        assert!(!ENABLE_CPU_MEMORY_ARENA);
        assert!(!ENABLE_MEMORY_PATTERN);
    }
}
use regex::Regex;
...
impl ParakeetModel {
    ...
    fn init_session<P: AsRef<Path>>(
        ...
        // Live chunks vary in duration. ORT's CPU arena and memory-pattern
        // caches retain each larger tensor shape for the full model lifetime,
        // causing resident memory to climb throughout multi-hour recordings.
        let providers = vec![CPUExecutionProvider::default()
            .with_arena_allocator(ENABLE_CPU_MEMORY_ARENA)
            .build()];
