...
pub async fn complete_onboarding<R: Runtime>(
    ...
    info!("Completing local-first onboarding");

    // Step 1: Save model configuration to SQLite database FIRST
    let pool = state.db_manager.pool();

    // Summaries are optional and use loopback Ollama when the user configures it.
    ...
    .await
    {
        error!("Failed to save local model config: {}", e);
        return Err(format!("Failed to save local model config: {}", e));
    }
    info!("Saved optional loopback Ollama configuration");

    // The pilot uses the signed-manifest multilingual Whisper artifact for JA/EN/KO.
