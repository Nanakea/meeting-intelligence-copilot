...
fn is_localhost_endpoint(endpoint: Option<&str>) -> bool {
    endpoint
        ...
        .map(|value| validate_endpoint_url(value).is_ok())
        .unwrap_or(true)
}
...
fn validate_endpoint_url(url: &str) -> Result<(), OllamaError> {
    ...
    let parsed = url::Url::parse(url).map_err(|_| {
        OllamaError::InvalidEndpoint("Use a plain loopback HTTP origin".to_string())
    })?;
        ...
        || !matches!(parsed.path(), "" | "/")
    {
        return Err(OllamaError::InvalidEndpoint(
            "Use a plain loopback HTTP origin".to_string(),
        ));
    }
...
mod endpoint_tests {
    ...
    #[test]
    fn accepts_only_plain_loopback_http_origins() {
        for allowed in [
            "http://localhost:11434",
            "http://127.0.0.1:11434/",
            "http://[::1]:11434",
        ] {
            assert!(validate_endpoint_url(allowed).is_ok(), "{allowed}");
        }
        for rejected in [
            ...
            assert!(validate_endpoint_url(rejected).is_err(), "{rejected}");
        }
    }
}
