...
pub async fn set_notification_settings(
    ...
    if let Some(manager) = manager_lock.as_ref() {
        manager
            ...
            .await
            .map_err(|_| "Failed to update notification settings".to_string())
    } else {
        Err("Notification manager not initialized".to_string())
    }
}
...
pub async fn request_notification_permission(
    ...
    if let Some(manager) = manager_lock.as_ref() {
        manager
            .request_permission()
            .await
            .map_err(|_| "Failed to request notification permission".to_string())
    } else {
        Err("Notification manager not initialized".to_string())
    }
}
...
pub async fn show_notification(
    ...
    if let Some(manager) = manager_lock.as_ref() {
        manager
            ...
            .await
            .map_err(|_| "Failed to show notification".to_string())
    } else {
        Err("Notification manager not initialized".to_string())
    }
}
...
pub async fn show_test_notification(
    ...
    if let Some(manager) = manager_lock.as_ref() {
        manager
            ...
            .await
            .map_err(|_| "Failed to show test notification".to_string())
    } else {
        Err("Notification manager not initialized".to_string())
    }
}
...
pub async fn set_manual_dnd(
    ...
    if let Some(manager) = manager_lock.as_ref() {
        manager
            ...
            .await
            .map_err(|_| "Failed to set manual DND".to_string())
    } else {
        Err("Notification manager not initialized".to_string())
    }
}
...
pub async fn set_notification_consent(
    ...
    if let Some(manager) = manager_lock.as_ref() {
        manager
            .set_consent(consent)
            .await
            .map_err(|_| "Failed to set notification consent".to_string())
    } else {
        Err("Notification manager not initialized".to_string())
    }
}
...
pub async fn clear_notifications(
    ...
    if let Some(manager) = manager_lock.as_ref() {
        manager
            .clear_notifications()
            .await
            .map_err(|_| "Failed to clear notifications".to_string())
    } else {
        Err("Notification manager not initialized".to_string())
    }
}
...
pub async fn initialize_notification_manager_manual(
    ...
    match initialize_notification_manager(app).await {
        ...
        Err(_) => {
            log_error!("Failed to initialize notification manager manually");
            Err("Failed to initialize notification manager".to_string())
        }
    }
}
...
pub async fn test_notification_with_auto_consent(
    ...
    if manager_lock.is_none() {
        drop(manager_lock);
        if initialize_notification_manager_manual(app.clone(), manager_state.clone())
            .await
            .is_err()
        {
            return Err("Failed to initialize notification manager".to_string());
        }
    } else {
        drop(manager_lock);
    }
    ...
    if let Some(manager) = manager_lock.as_ref() {
        ...
        if manager.set_consent(true).await.is_err() {
            log_error!("Failed to set notification consent");
        }
        if manager.request_permission().await.is_err() {
            log_error!("Failed to request notification permission");
        }
...
pub async fn get_notification_stats(
    ...
    if let Some(manager) = manager_lock.as_ref() {
        ...
        serde_json::to_value(stats)
            .map_err(|_| "Failed to serialize notification statistics".to_string())
    } else {
        Err("Notification manager not initialized".to_string())
    }
}
    ...
    if let Some(manager) = manager_lock.as_ref() {
        ...
        match initialize_notification_manager(app_handle.clone()).await {
            ...
            Err(_) => {
                ...
                let title = "Meeting Intelligence Copilot";
                ...
                log_info!("Using direct Tauri recording notification fallback");
                ...
                {
                    Ok(_) => {
                        log_info!("Successfully showed fallback recording notification");
                        Ok(())
                    }
                    Err(_) => {
                        log_error!("Failed to show fallback recording notification");
                        Err(anyhow::anyhow!("Failed to show recording notification"))
                    }
                }
            }
        }
