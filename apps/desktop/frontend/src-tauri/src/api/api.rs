    ...
    match store.get("authToken") {
        Some(token) => {
            if let Some(token_str) = token.as_str() {
                log_info!("Auth token is configured");
...
async fn make_api_request<R: Runtime, T: for<'de> Deserialize<'de>>(
    ...
    log_info!("Making {} request to the configured service", method);
    ...
    let response = request.send().await.map_err(|_| {
        log_error!("Configured service request failed");
        "Configured service request failed".to_string()
    })?;
    ...
    if !status.is_success() {
        log_error!("Configured service returned HTTP {}", status);
        return Err(format!("Configured service returned HTTP {}", status));
    }

    let response_text = response.text().await.map_err(|_| {
        log_error!("Configured service response could not be read");
        "Configured service response could not be read".to_string()
    })?;

    log_info!("Configured service response received");

    serde_json::from_str(&response_text).map_err(|_| {
        log_error!("Configured service response was invalid JSON");
        "Configured service response was invalid JSON".to_string()
    })
}
...
pub async fn api_get_meetings<R: Runtime>(
    ...
    match meetings {
        ...
        Err(_) => {
            log_error!("Error getting saved local meetings");
            Err("Saved meetings could not be loaded".to_string())
        }
    }
}
...
pub async fn api_search_transcripts<R: Runtime>(
    ...
    log_info!(
        "api_search_transcripts called (query characters: {}, auth configured: {})",
        query.chars().count(),
        auth_token.is_some()
    );
    ...
    match TranscriptsRepository::search_transcripts(pool, &query).await {
        ...
        Err(_) => {
            log_error!("Local transcript search failed");
            Err("Local transcript search failed".to_string())
        }
    }
}
...
pub async fn api_get_model_config<R: Runtime>(
    ...
    match SettingsRepository::get_model_config(pool).await {
        Ok(Some(config)) if matches!(config.provider.as_str(), "ollama" | "builtin-ai") => {
            log_info!(
        ...
        Ok(Some(_)) | Ok(None) => {
            log_warn!(
                "No approved local summary configuration found; returning loopback Ollama default"
            );
            Ok(Some(ModelConfig {
...
pub async fn api_save_model_config<R: Runtime>(
    ...
    if !matches!(provider.as_str(), "ollama" | "builtin-ai") {
        return Err("Only local summary providers are enabled in this build".to_string());
    }
    log_info!(
...
pub async fn api_get_transcript_config<R: Runtime>(
    ...
    match SettingsRepository::get_transcript_config(pool).await {
        Ok(Some(config)) if matches!(config.provider.as_str(), "localWhisper" | "parakeet") => {
            log_info!(
        ...
        Ok(Some(_)) | Ok(None) => {
            log_info!("No approved local transcript config found, returning verified default.");
            Ok(Some(TranscriptConfig {
...
pub async fn api_save_transcript_config<R: Runtime>(
    ...
    if !matches!(provider.as_str(), "localWhisper" | "parakeet") {
        return Err("Only local transcription providers are enabled in this build".to_string());
    }
    log_info!(
...
pub async fn api_delete_meeting<R: Runtime>(
    ...
    let _local_data_guard = crate::audio::common::acquire_engine_lifecycle_lock().await;
    log_info!(
    ...
    let meeting = MeetingsRepository::get_meeting_metadata(pool, &meeting_id)
        .await
        .map_err(|_| "Meeting data could not be prepared for deletion".to_string())?
        .ok_or_else(|| "Meeting not found or already deleted".to_string())?;
    crate::summary::service::SummaryService::cancel_summary(&meeting_id);
    crate::meeting_local_data::delete_meeting_folder(meeting.folder_path.as_deref())
    ...
    match MeetingsRepository::delete_meeting(pool, &meeting_id).await {
        ...
        Ok(false) => {
            ...
            Err("Meeting not found or already deleted".to_string())
        }
        Err(_) => {
            log_error!("Error deleting meeting {}", meeting_id);
            Err("Meeting could not be deleted".to_string())
        }
    }
}
...
pub async fn api_get_meeting<R: Runtime>(
    ...
    match MeetingsRepository::get_meeting(pool, &meeting_id).await {
        ...
        Err(_) => {
            log_error!("Error retrieving meeting {}", meeting_id);
            Err("Meeting could not be loaded".to_string())
        }
    }
}
...
pub async fn api_get_meeting_metadata<R: Runtime>(
    ...
    match MeetingsRepository::get_meeting_metadata(pool, &meeting_id).await {
        ...
        Err(_) => {
            log_error!("Error retrieving meeting metadata {}", meeting_id);
            Err("Meeting metadata could not be loaded".to_string())
        }
    }
}
...
pub async fn api_get_meeting_transcripts<R: Runtime>(
        ...
        .await
    {
        ...
        Err(_) => {
            log_error!("Error retrieving transcripts for meeting {}", meeting_id);
            Err("Meeting transcripts could not be loaded".to_string())
        }
    }
}
...
pub async fn api_save_meeting_title<R: Runtime>(
    ...
    match MeetingsRepository::update_meeting_title(pool, &meeting_id, &title).await {
        ...
        Err(_) => {
            log_error!("Failed to update saved local meeting");
            Err("Meeting could not be updated".to_string())
        }
    }
}
...
pub async fn api_save_transcript<R: Runtime>(
    ...
    log_info!(
        "api_save_transcript called (meeting title characters: {}, transcripts: {}, folder configured: {}, auth configured: {})",
    ...
    let transcripts_to_save: Vec<TranscriptSegment> = transcripts
        ...
        .map_err(|_| {
            ...
            "Invalid transcript data format".to_string()
        })?;

    // Log only aggregate timing metadata; transcript content stays local and out of logs.
    if let Some(first_seg) = transcripts_to_save.first() {
        log_debug!(
            "First parsed segment: text_chars={}, audio_start_time={:?}, audio_end_time={:?}, duration={:?}",
    ...
    if recording_session_id.as_deref().is_some_and(|session_id| {
        !crate::meeting_intelligence_pilot::valid_recording_session_id(session_id)
    }) {
        return Err("Invalid recording session identifier".to_string());
    }
    ...
    .await
    {
        ...
        Err(_) => {
            log_error!("Error saving local meeting transcript");
            Err("Meeting transcript could not be saved".to_string())
        }
    }
}
...
pub async fn open_meeting_folder<R: Runtime>(
    ...
    .fetch_optional(pool)
    .await
    .map_err(|_| "Meeting folder information could not be loaded".to_string())?;

    match meeting {
        Some(m) => {
            if let Some(folder_path) = m.folder_path {
                log_info!("Opening local recording folder for meeting {}", meeting_id);

                // Verify folder exists
                let path = std::path::Path::new(&folder_path);
                if !path.exists() {
                    log_warn!("Recording folder does not exist for meeting {}", meeting_id);
                ...
                #[cfg(target_os = "macos")]
                {
                    std::process::Command::new("open")
                        .arg(&folder_path)
                        .spawn()
                        .map_err(|_| "Recording folder could not be opened".to_string())?;
                }
                ...
                log_info!(
                    "Successfully opened recording folder for meeting {}",
                    meeting_id
                );
                Ok(())
            } else {
...
pub async fn open_external_url(url: String) -> Result<(), String> {
    ...
    {
        return Err("URL is not an allowed external address".to_string());
    }
    ...
    match result {
        ...
        Ok(_) | Err(_) => Err("The external address could not be opened".to_string()),
    }
}
            ...
            if status.is_success() {
                ...
                match serde_json::from_str::<serde_json::Value>(&response_text) {
                    Ok(json) => {
                        ...
                        log_warn!(
                            "⚠️ Endpoint returned 200 but response does not match the expected format (body bytes: {})",
                            response_text.len()
                        );
                ...
                log_warn!(
                    "⚠️ Custom OpenAI connection test failed with status {} (body bytes: {})",
                    status,
                    response_text.len()
                );
