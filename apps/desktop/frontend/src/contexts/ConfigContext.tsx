...
const isLocalSummaryProvider = (provider: string): provider is 'ollama' | 'builtin-ai' =>
  provider === 'ollama' || provider === 'builtin-ai';
    ...
    const loadTranscriptConfig = async () => {
      try {
        ...
        if (config) {
          ...
          const provider = config.provider === 'parakeet' ? 'parakeet' : 'localWhisper';
          setTranscriptModelConfig({
            provider,
            model: provider === 'parakeet'
              ? (config.model || 'parakeet-tdt-0.6b-v3-int8')
              : 'large-v3-turbo-q5_0',
    ...
    const fetchModelConfig = async () => {
      try {
        ...
        if (data && data.provider) {
          if (!isLocalSummaryProvider(data.provider)) {
            setModelConfig(prev => ({
              ...prev,
              provider: 'builtin-ai',
              model: '',
          ...
          // Apply only local providers from persisted configuration.
          setModelConfig(prev => ({
            ...prev,
  ...
  useEffect(() => {
    const setupListener = async () => {
      ...
      const unlisten = await listen<ModelConfig>('model-config-updated', (event) => {
        if (!isLocalSummaryProvider(event.payload.provider)) {
          setModelConfig(prev => ({ ...prev, provider: 'builtin-ai', model: '', apiKey: null }));
          return;
        }
  ...
  useEffect(() => {
    const loadDevicePreferences = async () => {
      try {
        ...
        console.log('No device preferences found or failed to load');
      }
    };
