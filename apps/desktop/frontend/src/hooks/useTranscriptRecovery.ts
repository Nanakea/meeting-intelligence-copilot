...
export function useTranscriptRecovery(): UseTranscriptRecoveryReturn {
  ...
  const checkForRecoverableTranscripts = useCallback(async () => {
    ...
    try {
        ...
        recentMeetings.map(async (meeting) => {
          if (meeting.folderPath) {
            try {
              ...
              console.warn('Failed to check local recovery audio');
      ...
      console.error('Failed to check for recoverable transcripts');
  ...
  const recoverMeeting = useCallback(async (meetingId: string): Promise<{ success: boolean; audioRecoveryStatus?: AudioRecoveryStatus | null; meetingId?: string }> => {
    ...
    try {
      ...
      if (folderPath) {
        try {
          ...
          audioRecoveryStatus = {
            ...
            message: 'Local audio recovery was unavailable'
          };
        }
      } else {
        audioRecoveryStatus = {
      ...
      const saveResponse = await storageService.saveMeeting(
        ...
        folderPath ?? null,
        metadata.meetingId.startsWith('meeting-intel-') ? metadata.meetingId : null,
      );
      ...
      try {
        ...
        console.warn('Failed to apply pinned summary language to recovered meeting');
      ...
      if (folderPath) {
        try {
          ...
          console.warn('Checkpoint cleanup failed (non-fatal)');
        }
      }
      ...
      console.error('Failed to recover meeting');
      throw new Error('The interrupted meeting could not be recovered');
    } finally {
  ...
  const deleteRecoverableMeeting = useCallback(async (meetingId: string): Promise<void> => {
    try {
      ...
      if (metadata?.folderPath) {
        await invoke('delete_unsaved_recovery_folder', {
      ...
      console.error('Failed to delete recoverable meeting');
      throw new Error('The interrupted meeting could not be deleted safely');
    }
  }, []);
