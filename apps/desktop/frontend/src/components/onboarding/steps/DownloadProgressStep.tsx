...
export function DownloadProgressStep() {
  ...
  const [transcriptionState, setTranscriptionState] = useState<DownloadState>({
    status: transcriptionDownloaded ? 'completed' : 'waiting',
    progress: transcriptionDownloaded ? 100 : 0,
    downloadedMb: 0,
    totalMb: 670,
    speedMbps: 0,
  });
  ...
  const handleRetryDownload = async () => {
    ...
    console.log('[DownloadProgressStep] Retrying verified transcription model download');
  ...
  useEffect(() => {
    ...
    startBackgroundDownloads({
      ...
      console.error('Failed to start transcription model download:', error);
      if (!transcriptionDownloaded) {
  ...
  // Listen to the signed-manifest Whisper download.
  useEffect(() => {
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      if (actuallyAvailable && !transcriptionDownloaded) {
  ...
  return (
    <OnboardingContainer
      ...
      description="You can start using Meeting Intelligence Copilot after downloading the Transcription Engine."
      step={3}
      ...
      <div className="flex flex-col items-center space-y-6">
        ...
        <div className="w-full max-w-lg space-y-4">
          {renderDownloadCard(
            ...
            transcriptionState,
            'Verified multilingual Whisper (~547 MiB)',
            'MiB'
          )}
