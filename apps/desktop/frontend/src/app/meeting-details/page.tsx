...
import {
  getMeetingIssueDraft,
  getMeetingIssueDrafts,
  type StoredMeetingIssueDraft,
...
function MeetingDetailsContent() {
  ...
  const [issueDraft, setIssueDraft] = useState<StoredMeetingIssueDraft | null>(null);
  const [structuredIssueDrafts, setStructuredIssueDrafts] = useState<StoredStructuredIssueDraft[]>([]);
    ...
    const fetchIssueDraft = async () => {
      try {
        const [legacy, structured] = await Promise.all([
          getMeetingIssueDraft(meetingId),
          getMeetingIssueDrafts(meetingId),
        ]);
        setIssueDraft(legacy);
        setStructuredIssueDrafts(structured);
      } catch {
        setIssueDraft(null);
        setStructuredIssueDrafts([]);
        toast.warning('Could not load the local issue draft');
      }
    };

    const loadData = async () => {
      try {
        await Promise.all([fetchMeetingSummary(), fetchIssueDraft()]);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return <PageContent
    ...
    structuredIssueDrafts={structuredIssueDrafts}
    onStructuredIssueDraftsChanged={setStructuredIssueDrafts}
  />;
}
