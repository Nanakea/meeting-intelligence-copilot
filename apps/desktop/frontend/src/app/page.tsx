...
import { RecordingPreflightDialog } from '@/components/RecordingPreflightDialog';
import {
...
export default function Home() {
  ...
  const { transcriptModelConfig, selectedDevices, selectedLanguage } = useConfig();
  ...
  const { handleRecordingStart: startRecordingAfterPreflight } = useRecordingStart(isRecording, setIsRecordingState);
  ...
  const requestRecordingStart = async () => {
    ...
    setPreflight(null);
    try {
      setPreflight(await getPilotPreflight(
        selectedLanguage ?? null,
        selectedDevices?.micDevice ?? null,
        selectedDevices?.systemDevice ?? null,
      ));
    } catch {
      toast.warning('Preflight checks are unavailable', {
        description: 'You can still start recording; ASK NOW may be unavailable.',
      });
      setShowPreflight(false);
      await startRecordingAfterPreflight();
    } finally {
  ...
  const continueAfterPreflight = async () => {
    ...
    try {
      ...
      toast.error('Recording could not start', {
        description: 'Check the microphone and meeting-output setup, then try again.',
      });
    }
  };
  ...
  useEffect(() => {
    const performStartupChecks = async () => {
      try {
        ...
        const retention = await getPilotSettings();
        await applyMeetingRetention(retention);

        // Saved meetings remain local until the user-selected retention policy
        // or an explicit delete action removes them.
  ...
  return (
    ...
    >
      ...
      />
      <div className="flex min-h-0 flex-1 overflow-hidden max-[700px]:flex-col">
        <TranscriptPanel
        ...
        <IntelligencePanel
          ...
          lang={['ja', 'en', 'ko'].includes(selectedLanguage) ? selectedLanguage as 'ja' | 'en' | 'ko' : null}
          enabled={recordingState.isRecording && ['ja', 'en', 'ko'].includes(selectedLanguage)}
          ingressStatus={recordingState.intelligenceIngressStatus}
