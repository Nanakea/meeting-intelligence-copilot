...
import { buildStartRecordingCommandArguments } from './recordingCommandArguments';
import type { StructuredIssueDraft } from './intelligenceContracts';

export type IntelligenceIngressStatus =
  | 'ready'
...
export interface RecordingState {
  ...
  intelligence_ingress_status: IntelligenceIngressStatus;
}
...
export interface RecordingStoppedPayload {
  ...
  meeting_name?: string;
  recording_session_id?: string;
  recording_save_status: 'saved' | 'failed';
  issue_drafts?: StructuredIssueDraft[];
}
...
export class RecordingService {
  ...
  /**
   * Get the opaque Meeting Intelligence session id for the active recording.
   * The human-readable meeting name is display metadata and is not an identity.
   */
  async getRecordingIntelligenceSessionId(): Promise<string | null> {
    return invoke<string | null>('get_recording_intelligence_session_id');
  }
  ...
  async startRecordingWithDevices(
    ...
    return invoke(
      ...
      buildStartRecordingCommandArguments(micDeviceName, systemDeviceName, meetingName),
    );
  }
