...
class IndexedDBService {
  ...
  async markMeetingSaved(meetingId: string): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      const getRequest = meetingsStore.get(meetingId);
      getRequest.onsuccess = () => {
        const meeting = getRequest.result;
        if (meeting) {
          meeting.savedToSQLite = true;
          meeting.lastUpdated = Date.now();
          meetingsStore.put(meeting);
        } else {
          const cursorRequest = transcriptsStore
            .index('meetingId')
            .openCursor(IDBKeyRange.only(meetingId));
  ...
  async deleteAllMeetings(): Promise<void> {
    if (!this.db) await this.init();
    const transaction = this.db!.transaction(['meetings', 'transcripts'], 'readwrite');
    transaction.objectStore('meetings').clear();
